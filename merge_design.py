...
# Need to replace the final section button that says "Vérifier mon éligibilité & Démo"
...
# 3. Add Modal HTML body just before JS scripts
modal_html = """
...
js_logic = """
        ...
        function openForm() {
            ...
            setTimeout(() => {
                document.getElementById('typeform-modal').classList.add('show');
            }, 10);
            nextStep(1); // Reset to first step
        }
        
        function closeForm(e) {
            if(e && e.target.id !== 'typeform-modal' && e.target.tagName !== 'BUTTON' && !e.target.closest('button')) return;
            document.getElementById('typeform-modal').classList.remove('show');
            setTimeout(() => {
